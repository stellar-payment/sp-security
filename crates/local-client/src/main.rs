...
use corelib::{
   mapper,
   security::{
      aes256_decrypt, aes256_encrypt, aes256_iv_decrypt, ecdh_generate_secret, generate_shared_key,
      hmac512_hash, hmac512_verify,
   },
};
...
//       .unwrap();
//    let master_pk = BASE64.decode(mpk.data.unwrap().public_key.as_bytes()).unwrap_or_else(|e| panic!("{e}"));
...
//       partner_id: "018b56fa-26af-7a99-8e4d-01fc3ed68cba".to_string(),
...
async fn get_master_keypair_by_hash(hash: String) -> MasterPKResponse {
   let res = reqwest::get(format!(
      "http://localhost:7780/ms/security/api/v1/keypairs/master/hash/{}",
      hash.clone()
   ))
   .await
   .unwrap();
   ...
   let data = res.json::<ApiResponse<MasterPKResponse>>().await.unwrap();
...
async fn main() {
   ...
   let payload = EncryptDataPayload {
      ...
      partner_id: "018bf191-9974-7974-b7dd-81f9eda96df3".to_string(),
   };

   let client = reqwest::Client::new();
   let enc_res = client
      .post("http://localhost:7780/ms/security/api/v1/payload/encrypt")
      .json(&payload)
      .send()
      .await
      .unwrap_or_else(|e| panic!("{e}"));

   let encrypted = enc_res
      .json::<ApiResponse<EncryptDataResponse>>()
      .await
      .unwrap()
      .data
      .expect("failed");
   ...
   let master_pk = BASE64
      .decode(
         get_master_keypair_by_hash(encrypted.secret_key)
            .await
            .public_key
            .as_bytes(),
      )
   ...
   // secret key
   let partner_pk = BASE64
      .decode("077EJnILqo6gtHMMp0UNg4ut2uP/2SsPVGxfFaeR0iM=".as_bytes())
      .unwrap_or_else(|e| panic!("{e}"));
   ...
   println!(
      "pk key: {} len: {}",
      BASE64.encode(&master_pk),
      mac_key.len()
   );
   println!(
      "sk key: {} len: {}",
      BASE64.encode(&partner_pk),
      mac_key.len()
   );
   ...
   let pt = aes256_iv_decrypt(enc_key, &iv, &ct).unwrap_or_else(|e| panic!("{e}"));
