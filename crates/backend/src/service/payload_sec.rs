...
pub struct PayloadSecurityService {
    ...
    ephemeral_master_repository: MasterEPKRepository,
    static_master_repository: MasterPKRepository,
}
...
pub trait PayloadSecurityServiceTrait {
    ...
    async fn get_keypair_by_hash(&mut self, hash: String)
    ...
    async fn _create_static_keypair(&mut self) -> Result<(SecretKey, String), KeypairError>;
...
impl PayloadSecurityServiceTrait for PayloadSecurityService {
    fn new(db: &Arc<Database>, cache: Cache) -> Self {
        Self {
            ...
            ephemeral_master_repository: MasterEPKRepository::new(cache),
            static_master_repository: MasterPKRepository::new(db),
        }
    }
    ...
    async fn encrypt_payload(
        ...
        let partner_data = self
            .partner_repository
            .find_partner_keypairs(
                Uuid::parse_str(&payload.partner_id)
                    .unwrap_or_else(|e| panic!("invalid uuidv7: {e}")),
            )
            .await
            .map_err(|e| SecurityError::GenericError(e.to_string()))?;

        let (secret_key, keyhash) = self
            ._create_static_keypair()
            .await
            .map_err(|e| SecurityError::GenericError(e.to_string()))?;

        let dec_public_key =
            aes256_decrypt(key, &partner_data.public_key).unwrap_or_else(|e| panic!("{e}"));
        ...
        let (enc_key, mac_key) =
            security::generate_shared_key(&shared_secret).map_err(SecurityError::from)?;
    ...
    async fn decrypt_payload(
        ...
        let secret_key = match self._get_keypair_by_hash(payload.keypair_hash).await {
            Ok(v) => v,
            Err(e) => {
                log::error!("{e}");
    ...
    async fn _create_keypair(&mut self) -> Result<(SecretKey, String), KeypairError> {
        ...
        return match self
            .ephemeral_master_repository
            .insert_keypair(payload)
            .await
        {
    ...
    async fn _create_static_keypair(&mut self) -> Result<(SecretKey, String), KeypairError> {
        let master_keypair_list = self
            .static_master_repository
            .find_keypairs()
            .await
            .map_err(|e| KeypairError::Yabai(e.to_string()))?;

        let master_data = master_keypair_list
            .choose(&mut OsRng)
            .ok_or(KeypairError::Yabai("no master key found".to_string()))?;
        ...
        let dec_secret_key = aes256_decrypt(key, &master_data.private_key)
            .unwrap_or_else(|e| panic!("{e}"));
        let secret_key = SecretKey::from_slice(&dec_secret_key).unwrap();

        let mut msg = master_data.private_key.clone();
        msg.extend(master_data.public_key.clone());
        let hashed = corelib::security::hmac256_hash(get("HASH_KEY").as_bytes(), &msg)
            .map_err(|e| KeypairError::Yabai(e.to_string()))?;

        let payload = EphemeralMasterKeyPair {
            public_key: master_data.public_key.clone(),
            private_key: master_data.private_key.clone(),
            keypair_hash: BASE64URL.encode(&hashed),
        };
        ...
        return match self
            ...
            .await
        {
            Ok(()) => Ok((secret_key, BASE64URL.encode(&hashed))),
            Err(e) => Err(KeypairError::CreationError(e.to_string())),
        };
    }
    ...
    async fn _get_keypair_by_hash(&mut self, hash: String) -> Result<SecretKey, KeypairError> {
        ...
        let meta = match self
            .ephemeral_master_repository
            .find_keypair_by_hash(hash.clone())
            .await
        {
            Ok(v) => v,
            Err(e) => match e {
    ...
    async fn get_keypair_by_hash(
        ...
        let decoded_key = BASE64
            .decode(get("DB_KEY").as_bytes())
            .map_err(|e| KeypairError::Yabai(format!("failed to decode key: {e}")))?;
        let key: GenericArray<u8, U32> = GenericArray::clone_from_slice(&decoded_key);
        ...
        // validate key
        let _ = SecretKey::from_slice(&ppk)
            .map_err(|e| KeypairError::IntegrityCheckFailed(e.to_string()))?;
        let _ = PublicKey::from_sec1_bytes(&pk)
            .map_err(|e| KeypairError::IntegrityCheckFailed(e.to_string()))?;
